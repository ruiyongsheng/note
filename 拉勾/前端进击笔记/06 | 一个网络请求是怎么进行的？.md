...
讲师回复： <iframe>，它能够将另一个HTML页面嵌入到当前页面中
...
讲师回复： 本地调试时，比如使用 npm run dev 起服务，本地地址常常为 localhost 或者是 127.0.0.1，这些地址如果直接发起请求是会跨域且可能被拒绝的，配置 host 很多时候是为了将支持跨域的域名映射到本地，从而可以正常地访问和进行本地调试
...
讲师回复： 代理只能解决本地开发的情况，本地开发更加常用的是配置 host 来解决