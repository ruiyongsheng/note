...
讲师回复： 可以把flutter doctor的结果贴一下后续，主要是想看下当前的flutter版本，因为flutter的某些版本在dartdoc上确实存在一些bug。这个问题，看起来是flutter dartdoc的问题。
...
若执行 dartdoc 报如下错误 ❌dartdoc failed: Top level package requires Flutter but FLUTTER_ROOT environment variable not set.需要配置 FLUTTER_ROOT 环境变量export FLUTTER_ROOT=你的 flutter 安装目录
...
讲师回复： 之前版本是dartanalyzer lib，最新版本已经修改为dart analyze，具体可以看下这个链接的说明：https://dart.dev/tools/dart-analyze，后续我将这块进行更新下。
...
讲师回复： 嗯，是的哈，随着版本的更新，有些信息是会有所不同，不过这两者都是可以的，flutter analyze 则是可以直接执行，简化了原来配置环境变量的过程。
...
讲师回复： 确实没有像前端一样可以自动化生成和保存的方式。你说要记住库和库版本，这点倒是可以去 https://pub.flutter-io.cn/ 这里搜索，有点像 npm 包库，不过这里还是需要使用英文关键词，比如我们像搜索 location，获取地理位置的，还是能够找到的。
...
讲师回复： 主要看你们习惯，代码缩进 2 或者 4 个都可以，我建议 2 个。
...
讲师回复： 运行期间有没有报错信息，其次可以把flutter doctor的结果贴一下。如果是和下面同一个问题，可以看看换下最新版本的flutter试试。
...
讲师回复： 你看下你的dartdoc版本，或者flutter版本，在某个版本是存在这样的bug，你可以试着升级下最新版本的flutter sdk。
...
讲师回复： 你看下flutternsdl版本，有个版本的确有这个问题，用最新的一个版本再尝试下，新版本是已经修复了这个问题的。
...
讲师回复： 一般 IDE 可以自带自动化检测，可以参考这个文档。https://flutter.cn/docs/get-started/editor
...
讲师回复： 这部分可能需要你提供更多的信息才知道，首先检查下在文档生成过程中是否报错，特别是你提到的失败的这个类。
...
讲师回复： 如果你是mac，在命令行窗口运行下ls $PATH，看下有没有/dart-sdk/bin目录，如果没有根据我们第3课时的方法去配置下。如果是windows，直接去打开环境变量确认下是否有dart-sdk/bin路径。
...
讲师回复： 你确认下是否在当前项目目录，具体可能得结合你实际场景来看。