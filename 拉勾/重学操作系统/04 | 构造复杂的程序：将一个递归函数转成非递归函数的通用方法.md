...
讲师回复： 我举的是一种实现。有的语言支持case是一个表达式，这种情况还需要用到专门的数据结构。
...
编辑回复： 每个模块最后的加餐就是各模块的思考题详解，04课时的习题讲解在“加餐 | 练习题详解（一）”哦～
...
讲师回复： 压栈是两条指令。一条是设置SP指向地址的值，一条是操作SP。出栈只需一条指令。 前者加1是因为函数调用本身也是一条指令。后者加1是因为返回地址也可以出栈。
...
讲师回复： 是的，函数内部也要压栈。具体压栈的个数记录在一个叫做符号表的数据结构中。静态编译的话符号表最后不打包在程序里。动态编译比如Java需要一个元数据区。
...
讲师回复： 这里就有很多实现了，比如基于虚函数表、基于闭包、基于哈希表等等非常多。
...
编辑回复： 哪一块比较难懂？可以描述具体一点，这样老师才能帮你解答哦
