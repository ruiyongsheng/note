...
讲师回复： 课程中已经提到过，各个插件都是单独的进程，可以从浏览器的任务管理器中看到它们有不同的进程ID，也可以使用命令行程序进程查看，比如 Mac 下 ps -ef|grep 扩展程序进程号
...
讲师回复： 很可能是的，可以通过浏览器的任务管理器进行查看。 Chrome 浏览器中，通过a标签或者 window.open() 函数来打开同一个根主机名下的网站（ 比如在 https://kaiwu.lagou.com 中打开 https://lagou.com），不会创建渲染进程而是用的多线程