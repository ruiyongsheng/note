...
![Drawing 5.png](https://s0.lgstatic.com/i/image6/M00/1A/35/CioPOWBLG6aASuNtAAURvlMLkr8118.png)
...
讲师回复： 通过loacalstorage可以做到，目前我们是这么做的：每个页面都有一个唯一的key，会存储于cookie中，第一次访问时，这个key值+1，后续如果数字大于1，就是二次访问，反之首次访问
...
讲师回复： localStorage是可以的，我们目前用的是cookie，每个页面都有一个根据url生成的key值，会存储于cookie中，第一次访问时，这个key值+1，后续如果数字大于1，就是二次访问，反之首次访问
...
讲师回复： 也是一个思路，我们目前是这么做的：每个页面都有一个根据url生成的key值，会存储于cookie中，第一次访问时，这个key值+1，后续如果数字大于1，就是二次访问，反之首次访问
...
讲师回复： 其中一个目的是这个，另外一个目的是为了了解用户首次访问的首屏时间