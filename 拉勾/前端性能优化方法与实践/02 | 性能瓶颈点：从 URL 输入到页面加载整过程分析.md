...
讲师回复： 公认口碑不错的没看到，你可以看看这两个 https://github.com/kisslove/web-monitoring/ https://github.com/wangweianger/zanePerfor
...
讲师回复： 占位图是设置了尺寸的，大小也比较固定，加载真实图片后，未设置最大宽高等限制条件
...
讲师回复： 一般来说可以做一些预解析，或者通过 webview 设置做一些事情，重方案的话可以借助客户端做一些缓存处理，10 讲会提到
...
讲师回复： 赞动手实践及提出问题，这里表述上是有点问题，JSmodern 浏览器针对这种情况做了 Paint 的优化，先触发一次 Paint 将前面内容输出。inline 的 script 标签和 head 中的 script 标签不会触发 Paint。
...
讲师回复： 是的，如果全站都是 http2.0，就没有必要了，后面会有讲到
...
节流和防抖，主要目的是渲染阶段和交互阶段的优化手段，一方面是减少重排和重绘的次数，一方面是减少JavaScript的执行频繁导致页面卡顿的情况。