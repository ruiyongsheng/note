...
在 HTTP/2 中引入了`多路复用`的技术，这个技术可以只通过一个 TCP 连接就可以传输所有的请求数据。`多路复用`很好的解决了浏览器限制同一个域名下的请求数量的问题，同时也接更容易实现全速传输，毕竟新开一个 TCP 连接都需要慢慢提升传输速度。

大家可以通过[该链接](https://http2.akamai.com/demo) 感受下 HTTP/2 比 HTTP/1 到底快了多少。

![](https://user-gold-cdn.xitu.io/2020/5/30/1726579780d8a9ce?w=782&h=500&f=png&s=367082)
在 HTTP/1 中，因为队头阻塞的原因，你会发现发送请求是长这样的

![](https://user-gold-cdn.xitu.io/2020/5/30/172657a0ebcfe009?w=788&h=998&f=png&s=436919)

在 HTTP/2 中，因为可以复用同一个 TCP 连接，你会发现发送请求是长这样的

![](https://user-gold-cdn.xitu.io/2020/5/30/172657a6e33eb9b9?w=796&h=562&f=png&s=177206)
...
![](https://user-gold-cdn.xitu.io/2020/5/30/172657ae443525c2?w=768&h=480&f=png&s=126590)
...
![](https://user-gold-cdn.xitu.io/2020/5/30/172657bb0bb7a283?w=814&h=282&f=png&s=42226)
...
因为 HTTP/2 使用了`多路复用`，一般来说同一域名下只需要使用一个 TCP 连接。当这个连接中出现了丢包的情况，那就会导致 HTTP/2 的表现情况反倒不如 HTTP/1 了。
