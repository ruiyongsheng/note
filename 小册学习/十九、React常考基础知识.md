...
![](https://user-gold-cdn.xitu.io/2020/5/29/1725e4aca188ceb9?w=593&h=639&f=png&s=216607)
...
![](https://user-gold-cdn.xitu.io/2020/5/29/1725e4bdb7df9dbc?w=596&h=319&f=png&s=169070)
...
```！
PS: 下文中的state指代了 state 及 props
```
在 `shouldComponentUpdate` 函数中我们可以通过返回布尔值来决定当前组件是否需要更新。这层代码逻辑可以是简单地浅比较一下当前 `state` 和之前的 `state` 是否相同，也可以是判断某个值更新了才触发组件更新。一般来说不推荐完整地对比当前 `state` 和之前的 K 是否相同，因为组件更新触发可能会很频繁，这样的完整对比性能开销会有点大，可能会造成得不偿失的情况。
