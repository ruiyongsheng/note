...
```
前面都是在讲述`Promise`的一些优点和特点，
缺点就是无法取消`Promise`,错误需要通过回调函数捕获。
...
* 因为`await` 是异步操作，后来的表达式不返回`Promise`的话，就会包装成`Promise.resolve(返回值)`，然后会执行函数外的同步代码
* 同步代码执行完毕后开始执行异步diamante，将保存下来的值拿出来使用，这时候`a = 0 + 10`
